...
function login(havetoken) {
	...
	$.post("http://www.iglaset.se/user_sessions.xml", { "user_session[username]": username, "user_session[password]": password }, function(xml) {
...
function get_categories() {
	if (!categories_populated) {
		$.mobile.loading('show');
		$("#categories").html("");
		$.get("http://www.iglaset.se/categories.xml", function(xml) {
...
function get_stores() {
	if (!stores_populated) {
		...
		$("#stores").html("");
		var a=0;
		$.get("http://www.iglaset.se/ajax/get_stores", function(xml) {
...
function view_articles_by_cat(cat, page) {
	...
	$.get("http://www.iglaset.se/articles.xml/cat/?user_credentials="+window.localStorage.getItem("token")+"&page="+page+"&category="+cat+"&order_by="+sort_by, function(xml) {
	 	article_line(xml, "articles");
		$("#search-res-more-button").show();		
	 	page = parseInt(page)+1;
	 	var tot_art = $(xml).find('articles').attr("total_articles");
...
function basement_list(page) {
	...
	$.mobile.loading('show');
	$.get("http://www.iglaset.se/users/"+uid+".xml?show=basement&user_credentials="+window.localStorage.getItem("token")+"&page="+page, function(xml) {
...
function purchase_list(page) {
	...
	if (purchase_no_data || page != 1) {
		...
		if ( uid>0) {
			...
			$.mobile.loading('show');
			$.get("http://www.iglaset.se/users/"+uid+".xml?show=purchase&user_credentials="+window.localStorage.getItem("token")+"&page="+page, function(xml) {
...
function get_recommended_articles(page, cat) {
	...
	//	$.mobile.changePage("#recommendations-res-page");
		...
		$.get("http://www.iglaset.se/users/mypage.xml?show=recommendations&"+catfilter+"page="+page+"&order_by=recommendation&user_credentials="+window.localStorage.getItem("token"), function(xml) {			
	...
	if (window.localStorage.getItem("store_id")) {
		...
		var sb_art_ids = sb_art_ids.filter( onlyUnique )
			...
			$(sb_art_ids).each(function(v,k) {
				if (k != "false") {
					gets.push($.get("http://www.iglaset.se/ajax/get_store_stock?varunr="+k, function (xml) {
...
function get_producer() {
	...
	$.get("http://www.iglaset.se/producers/"+producer_id+".xml", function(xml) {
...
function list_scan(list, incdec) {
   ...
   $("#scan-choose-list2").html("");
	...
	cordova.plugins.barcodeScanner.scan(
		function (result) {
			$.get("http://www.iglaset.se/barcodes/show_by_ean/"+result.text+".xml?user_credentials="+window.localStorage.getItem("token"), function(xml) {
...
function update_list(list, incdec, artid, close_popups) {
	...
	if (list==1) {
		if (incdec==1) {
			...
			$.get("http://www.iglaset.se/articles/"+artid+"/list_modify.xml?list_type=purchase&inc=1&user_credentials="+window.localStorage.getItem("token"), function(xml) {
			 if (close_popups){
			...
			$.get("http://www.iglaset.se/articles/"+artid+"/list_modify.xml?list_type=purchase&dec=1&user_credentials="+window.localStorage.getItem("token"), function(xml) {
			 if (close_popups){
			  	$('#scan-choose-popup').popup("close");
			 }			 
			 	purchase_no_data = true;			  
		...
		if (incdec==1) {
			$.get("http://www.iglaset.se/articles/"+artid+"/list_modify.xml?list_type=basement&inc=1&user_credentials="+window.localStorage.getItem("token"), function(xml) {
			 if (close_popups){
			...
			$.get("http://www.iglaset.se/articles/"+artid+"/list_modify.xml?list_type=basement&dec=1&user_credentials="+window.localStorage.getItem("token"), function(xml) {
			 if (close_popups){
			  	$('#scan-choose-popup2').popup("close");
			 }
				basement_list(1);
